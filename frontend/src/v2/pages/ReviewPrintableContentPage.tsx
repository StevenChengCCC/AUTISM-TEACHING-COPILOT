...
const designOptions=[
  {id:"calm-blue",label:"Calm blue",color:"blue",note:"Clear and low-distraction"},
  {id:"playful-green",label:"Playful green",color:"green",note:"Friendly and encouraging"},
  {id:"warm-gold",label:"Warm gold",color:"gold",note:"Warm classroom energy"},
];
...
export function ReviewPrintableContentPage({ lessonPackage,onBack,onFeedback }:{ lessonPackage:LessonPackage|null;onBack:()=>void;onFeedback:(message:string)=>void }) {
  ...
  return <section><div className="v2-page-heading"><h1>Review Printable Content</h1><p>Open a material, inspect the print layout, and make edits before export.</p></div><div className="v2-print-layout">
    ...
    <Card className="v2-edit-material"><div className="v2-preview-head"><h2>Choose a finished design</h2>{approved&&<Tag tone="green">Approved</Tag>}</div><p className="v2-design-intro">Each option is already print-ready. Choose one; editing is optional.</p><div className="v2-design-options">{designOptions.map((option)=><button type="button" className={`v2-design-option v2-design-option--${option.color} ${theme===option.color?"is-active":""}`} onClick={()=>{setTheme(option.color);setDirty(true);}} key={option.id}><span><i/><i/><i/></span><b>{option.label}</b><small>{option.note}</small>{theme===option.color&&<em>✓ Selected</em>}</button>)}</div>
