...
export function ReviewLearnerPage({ learnerId,isNew,onContinue,onBack,onFeedback }:{ learnerId:string;isNew:boolean;onContinue:()=>void;onBack?:()=>void;onFeedback:(message:string)=>void }) {
  ...
  const retryExtraction=async()=>{setIsLoading(true);setLoadError(null);try{const value=await lessonKitApi.analyzeLearnerProfile(learnerId);setExtraction(value);setForm(profileFromLearner(value.learner));}catch(error){setLoadError(error instanceof Error?error.message:"Learner information is temporarily unavailable.");}finally{setIsLoading(false);}};
  if(loadError||!extraction)return <div className="v2-load-error" role="alert"><Card><span className="v2-load-error__icon" aria-hidden="true">!</span><h2>AI needs another try</h2><p>Your uploaded record is safe. The AI service did not finish the learner summary this time.</p><Button onClick={()=>void retryExtraction()}>Try profile analysis again</Button></Card></div>;
  ...
  if(isNew&&extraction.analyzedRecordCount>0&&factors.length===0&&(learner.profileSignals??[]).length===0)return <div className="v2-load-error" role="status"><Card><span className="v2-load-error__icon" aria-hidden="true">✦</span><h2>Record ready for AI analysis</h2><p>Your reviewed record is safely saved. Start the paid AI analysis to prepare the learner summary.</p><Button onClick={()=>void retryExtraction()}>Prepare learner summary</Button></Card></div>;
