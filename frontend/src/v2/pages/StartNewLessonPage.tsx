  ...
  return <>
    ...
    <div className="v2-start-grid">
      <Card className="v2-start-card"><h2><span className="v2-heading-icon">♙</span> Use Existing Learner</h2><label className="v2-search"><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Search learners by code" /></label>
        <div className="v2-learner-list">{loading?<p className="v2-empty-state">Loading learners…</p>:learners.length===0?<p className="v2-empty-state">No learners match this search.</p>:learners.map((learner)=><button key={learner.id} className={selectedId===learner.id?"is-selected":""} onClick={()=>setSelectedId(learner.id)}><span className="v2-list-avatar">{learner.avatar}</span><span className="v2-list-copy"><strong>{learner.code}</strong><small>{learner.age>0?`Age ${learner.age}`:"Age to confirm"}</small></span><span className="v2-list-tags">{learner.tags.slice(0,2).map((tag,index)=><Tag tone={index?"green":"blue"} key={tag}>{tag}</Tag>)}</span><span className="v2-chevron">›</span></button>)}</div>
