...
function PackageContentPreview({plan,busy,additionalMaterial,onAdditionalMaterial,onAdjust,onBack}:{plan:NonNullable<AIChatState["draft"]["packageContentPlan"]>;busy:boolean;additionalMaterial:string;onAdditionalMaterial:(value:string)=>void;onAdjust:(action:"set_optional"|"set_companion"|"add_material",materialType:string,included?:boolean)=>Promise<void>;onBack:()=>void}){
  return <Card className="v2-content-plan"><header><div><small>COMPLETE PACKAGE PREVIEW</small><h3>{plan.estimatedArtifactCount} artifacts · about {plan.estimatedPageCount} pages</h3></div><button type="button" onClick={onBack}>Return to selections</button></header>
    <section><h4>Your selected materials</h4>{plan.teacherSelectedCore.map((item)=><div key={item.materialRequestId}><strong>{materialLabel(item.materialType)}</strong><small>{item.reason}</small></div>)}</section>
    <section><h4>Automatically added because they are needed</h4>{plan.requiredCompanions.map((item)=><div key={item.materialType}><label><input type="checkbox" checked={item.included} disabled={busy||!item.canTeacherRemove} onChange={(event)=>void onAdjust("set_companion",item.materialType,event.target.checked)}/><strong>{materialLabel(item.materialType)}</strong></label><small>{item.reasonRequired}</small><details><summary>Why this was added</summary><p>{item.goalRequirement}{item.removalWarning?` · ${item.removalWarning}`:""}</p></details></div>)}</section>
    {plan.optionalEnrichments.length>0&&<section><h4>Optional materials</h4>{plan.optionalEnrichments.map((item)=><label key={item.materialType}><input type="checkbox" checked={item.defaultIncluded} disabled={busy} onChange={(event)=>void onAdjust("set_optional",item.materialType,event.target.checked)}/><span><strong>{materialLabel(item.materialType)}</strong><small>{item.reasonSuggested} · about {item.estimatedPages} page{item.estimatedPages===1?"":"s"}</small></span></label>)}</section>}
    {plan.excludedMaterials.length>0&&<section><h4>Excluded</h4>{plan.excludedMaterials.map((item)=><div key={item.materialType}><strong>{materialLabel(item.materialType)}</strong><small>{item.reasonExcluded}</small></div>)}</section>}
    <footer><select value={additionalMaterial} onChange={(event)=>onAdditionalMaterial(event.target.value)}>{addableMaterials.filter((item)=>![...plan.teacherSelectedCore.map((value)=>value.materialType),...plan.requiredCompanions.map((value)=>value.materialType),...plan.optionalEnrichments.map((value)=>value.materialType)].includes(item)).map((item)=><option key={item} value={item}>{materialLabel(item)}</option>)}</select><Button variant="secondary" disabled={busy} onClick={()=>void onAdjust("add_material",additionalMaterial,true)}>Add another supported material</Button></footer>
  </Card>;
}
