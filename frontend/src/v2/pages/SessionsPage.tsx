...
export function SessionsPage({onNewSession,onResume,onFeedback}:{onNewSession:()=>void;onResume:(session:LessonSession)=>void;onFeedback:(message:string)=>void}) {
  ...
  return <section>
    ...
    <div className={`v2-sessions-layout${completing?" is-recording":""}`}>{!completing&&<Card><h2>Recent Sessions</h2><label className="v2-search"><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Search sessions by learner or goal"/></label><div className="v2-session-table"><div className="v2-table-head"><span>LEARNER</span><span>GOAL</span><span>STATUS</span><span>LAST ACTIVITY</span></div>{shown.map((session)=><button key={session.id} onClick={()=>{setSelectedId(session.id);setCompleting(false);}} className={selected?.id===session.id?"is-selected":""}><strong>{learner(session.learnerId)?.code}</strong><span>{session.goal}</span><Tag tone={tone[session.status]}>{session.status.replace("_"," ")}</Tag><span>{formatLocalDateTime(session.updatedAt)} &nbsp;›</span></button>)}</div></Card>}
      {selected&&<Card className="v2-session-preview">{completing?<SessionCompletionForm sessionId={selected.id} onCancel={()=>setCompleting(false)} onCompleted={(outcome)=>void completed(outcome)}/>:<>
        ...
        <dl className="v2-session-details"><div><dt>◎ Goal</dt><dd>{selected.operationalizedGoal||selected.goal}</dd></div><div><dt>◷ Last activity</dt><dd>{formatLocalDateTime(selected.updatedAt)}</dd></div></dl>
