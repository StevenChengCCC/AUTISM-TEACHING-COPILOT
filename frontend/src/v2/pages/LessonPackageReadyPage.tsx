...
export function LessonPackageReadyPage({ lessonPackage,onModify,onReview,onEdit,onStartOver,onSave,onFeedback }:{ lessonPackage:LessonPackage|null;onModify:()=>void;onReview:()=>void;onEdit:()=>void;onStartOver:()=>void;onSave:(value:LessonPackage)=>void;onFeedback:(message:string)=>void }) {
  ...
  useEffect(()=>{
    ...
    let cancelled=false;
    const refresh=async()=>{try{const materials=await lessonKitApi.getGeneratedMaterials(lessonPackage.id);if(!cancelled)onSave({...lessonPackage,materials});}catch{/* Keep the package usable while artwork continues in the background. */}};
  ...
  const blocked=lessonPackage.safetyReview?.status==="blocked"||lessonPackage.status==="validation_failed";
  ...
  return <section><div className="v2-page-heading"><h1>Lesson Package Ready</h1><p>Review the generated teaching package before exporting or printing.</p></div>{localMock&&<div className="v2-generation-state v2-generation-state--mock" role="status"><strong>Local demo output</strong><span>This package was created by the deterministic local provider. It is not a successful external AI result and still requires teacher review.</span></div>}{blocked&&<div className="v2-generation-state v2-generation-state--blocked" role="alert"><strong>Review required</strong><span>This package did not pass the instructional safety gate. Modify the flagged content before printable approval.</span></div>}
    <div className="v2-package-layout"><div><Card className="v2-summary-strip"><div><span>{learner?.avatar??"👦🏻"}</span><small>Learner</small><strong>{learner?.code??"Learner"}</strong></div><div><span>◎</span><small>Goal</small><strong>{goalTitle}</strong></div><div><span>◷</span><small>Duration</small><strong>{lessonPackage.duration}</strong></div><div><span>🚙</span><small>Theme</small><strong>{lessonPackage.theme}</strong></div></Card>
      ...
      <TeacherHandoffExportPanel lessonPackage={lessonPackage} onPackageChange={onSave} onFeedback={onFeedback}/>
    </div><Card className="v2-export-panel"><h2>Next steps</h2><p>Edit the generated lesson before reviewing printable materials.</p>{[{id:"pdf",label:"Printable PDF"},...lessonPackage.materials.map((item)=>({id:item.id,label:item.title}))].map((item)=><label key={item.id}><input type="checkbox" checked={exports[item.id]!==false} onChange={()=>toggle(item.id)}/><span>▧</span>{item.label}</label>)}<Button fullWidth onClick={onModify}>Modify Lesson Content</Button><Button variant="secondary" fullWidth disabled={blocked} onClick={onReview}>Review Printable Content</Button><Button variant="secondary" fullWidth onClick={onEdit}>Back to Edit Lesson</Button></Card></div>
