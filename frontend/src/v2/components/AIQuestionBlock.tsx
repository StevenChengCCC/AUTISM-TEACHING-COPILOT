...
export function AIQuestionBlock({ question,onAnswer,busy=false }:{ question:AIQuestion;onAnswer:(ids:string[],customAnswer?:string,saveForFuture?:boolean)=>void|Promise<void>;busy?:boolean }) {
  ...
  const prompt=question.field==="scenarios"?"Which classroom situations should this lesson cover?":question.prompt;
  const helperText=question.field==="scenarios"?"Choose up to three routines or group formats where the skill is likely to be used.":question.helperText;
  return <div className="v2-question"><div className="v2-bot">✦</div><div className="v2-question-body">
    <div className="v2-question-title"><strong>{prompt}</strong></div>
    {helperText&&<p className="v2-question-helper">{helperText}</p>}
    {question.inputType!=="free_text"&&<div className={`v2-option-list ${question.inputType==="single_select"||question.maxSelections===1?"is-single":""}`}>{question.options.map((option)=><div className="v2-option-item" key={option.id}><OptionChip option={option} selected={question.selectedOptionIds.includes(option.id)} disabled={busy} onToggle={()=>toggle(option.id)}/>{question.selectedOptionIds.includes(option.id)&&(option.reason||option.affects?.length||option.unsupportedReason)&&<small className={option.supported===false?"v2-option-warning":"v2-option-reason"}>{option.unsupportedReason||option.reason}{option.profileFactorIds?.length?" · Learner-profile informed":""}{option.affects?.length?` · Affects ${option.affects.join(", ")}`:""}{option.supported===false&&!option.savedForFuture&&<button type="button" disabled={busy} onClick={()=>void onAnswer(question.selectedOptionIds,option.value,true)}>Save as future request</button>}{option.savedForFuture&&" · Saved for future use; it will not be generated"}</small>}</div>)}<div className="v2-option-list__actions">{canSelectAll&&<button type="button" disabled={busy} className={allSelected?"is-custom":""} onClick={()=>void onAnswer(allSelected?[]:selectableIds,"")}>{allSelected?"Clear all":"Select all"}</button>}{question.allowCustomAnswer&&<button type="button" disabled={busy} className={showCustom?"is-custom":""} onClick={()=>setShowCustom(true)}>＋ Write my own</button>}</div></div>}
