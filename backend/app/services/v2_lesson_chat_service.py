...
class V2LessonChatService:
    greeting = "Tell me what you want to teach today, and I’ll help turn it into a lesson kit."
    ...
    def start(self, learner_id: str) -> AIChatState:
        ...
        chat = AIChatState(
            ...
            messages=[AIMessage(id=self.repos.next_id("message"), role="assistant", content=self.greeting)],
            questions=[],
    ...
    def submit_request(self, conversation_id: str, content: str) -> AIChatState:
        ...
        chat.messages.append(AIMessage(id=self.repos.next_id("message"), role="teacher", content=clean))
        if not chat.questions:
            ...
            chat.draft.custom_notes = " ".join(filter(None, [chat.draft.custom_notes, clean]))
            response = "Thanks. I’ve kept your lesson choices and added that note to the draft."
        chat.messages.append(AIMessage(id=self.repos.next_id("message"), role="assistant", content=response))
        chat.can_generate = bool(chat.questions) and all(self._answered(item) for item in chat.questions)
        return self.repos.chats.save(chat)
    ...
    def update_answer(self, conversation_id: str, question_id: str, payload: QuestionAnswerUpdate) -> AIChatState:
        chat = self._get(conversation_id)
        question = next((item for item in chat.questions if item.id == question_id), None)
        if not question:
            raise NotFoundError("AI question not found")
        selected = [item for item in payload.selected_option_ids if any(option.id == item for option in question.options)]
        if question.input_type == "single_select":
            selected = selected[-1:]
        elif question.max_selections is not None:
            selected = selected[: question.max_selections]
        question.options = [option for option in question.options if option.source != "teacher_custom"]
        question.custom_answer = payload.custom_answer.strip()
        if question.custom_answer:
            custom_id = f"custom-{question.id}"
            question.options.append(AIQuestionOption(id=custom_id, label=question.custom_answer, value=question.custom_answer, icon="✎", source="teacher_custom"))
    ...
    def _answered(question: AIQuestion) -> bool:
        return not question.required or bool(question.selected_option_ids or question.custom_answer.strip())

    @staticmethod
    def _apply_answer(draft: LessonDesignDraft, question: AIQuestion) -> None:
        values = [option.value for option in question.options if option.id in question.selected_option_ids]
        if question.field == "responseLevel":
            ...
            if "ask for help" in draft.goal_text.lower() and draft.response_level:
                draft.goal_text = f"Learner will ask for help using a {draft.response_level.lower()}."
